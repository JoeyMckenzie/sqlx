...
use sqlx::{PgPool, FromRow, query_file, query_file_as, query_file_unchecked};
...
async fn main() -> anyhow::Result<()> {
    ...
    query_file!("queries/insert_seed_data.sql").execute(&pool).await?;

    // we can also use `query_file_as!()` similarly to `query_as!()` to map our database models
    let posts_with_authors = query_file_as!(PostWithAuthorQuery, "queries/list_all_posts.sql").fetch_all(&pool).await?;
